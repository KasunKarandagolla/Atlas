...
from .._serialization import FrozenMap, decimal_value, sha256_json, timestamp
...
class UniverseObservationV2:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.required_bars_present) is not bool or type(self.open_position) is not bool or type(self.active_watch) is not bool:
        ...
        health_available = self.received_at_ns if self.source_health_available_at_ns is None else self.source_health_available_at_ns
...
class DynamicUniverseRuntimeV2:
    ...
    def __init__(self, *, min_observed_days: int = 30, min_turnover_usd: Decimal = Decimal("10000000"), max_spread_bps: Decimal = Decimal("10")) -> None:
    ...
    def build_snapshot(
        ...
        *,
        ...
        for item in observations:
            ...
            strategies = {
                policy_id: StrategyEligibilityV2(
                    EligibilityStatusV2.ELIGIBLE if item.observed_days >= required_days and data_eligible else EligibilityStatusV2.NOT_ESTIMABLE,
                    None if item.observed_days >= required_days and data_eligible else "POLICY_HISTORY_OR_DATA_REQUIREMENT_NOT_MET",
                )
        ...
        for item in tier0:
            ...
            tiers[key] = (
                ComputeTierV2.TIER_3 if key in tier3_keys
                else ComputeTierV2.TIER_2 if key in tier2_keys
                else ComputeTierV2.TIER_1 if any(entry.key == key and entry.scanner_eligible for entry in entries)
        ...
        envelope = ArtifactEnvelope(1, f"universe-{decision_slot_ns}-{sha256_json(artifact_refs)[:16]}", created_at_ns, created_at_ns, "dynamic-universe-v1", artifact_refs)
        universe = UniverseContractV2(
            envelope,
            "dynamic-universe-v1",
            decision_slot_ns,
...
def subscription_channels_for_tier(tier: ComputeTierV2) -> tuple[SubscriptionChannelV2, ...]:
    ...
    by_tier = {
        ...
        ComputeTierV2.TIER_1: (
            SubscriptionChannelV2.KLINE_15M, SubscriptionChannelV2.KLINE_1H, SubscriptionChannelV2.KLINE_4H,
            SubscriptionChannelV2.BOOK_TICKER, SubscriptionChannelV2.MARK_INDEX, SubscriptionChannelV2.FUNDING,
            SubscriptionChannelV2.OPEN_INTEREST,
        ),
        ComputeTierV2.TIER_2: (
            SubscriptionChannelV2.KLINE_15M, SubscriptionChannelV2.KLINE_1H, SubscriptionChannelV2.KLINE_4H,
            SubscriptionChannelV2.BOOK_TICKER, SubscriptionChannelV2.MARK_INDEX, SubscriptionChannelV2.FUNDING,
            SubscriptionChannelV2.OPEN_INTEREST, SubscriptionChannelV2.TRADES,
        ),
