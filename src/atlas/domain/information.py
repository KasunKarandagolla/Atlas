...
class InformationContract:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.availability_class, AvailabilityClass):
            raise ValueError("availability_class invalid")
        for n in (
            "source_event_at_ns",
    ...
    def _validate(self):
        s = self
        if (
            s.source_event_at_ns is not None
            and s.source_published_at_ns is not None
            and s.source_published_at_ns < s.source_event_at_ns
        ):
            raise ValueError("source_published_at cannot precede source_event_at")
        if (
            s.source_published_at_ns is not None
            and s.received_at_ns is not None
            and s.received_at_ns < s.source_published_at_ns
        ):
            raise ValueError("received_at cannot precede source_published_at")
        if s.received_at_ns is not None and s.available_at_ns is not None and s.available_at_ns < s.received_at_ns:
            raise ValueError("available_at cannot precede received_at")
        if s.processed_at_ns is not None and s.available_at_ns is not None and s.available_at_ns < s.processed_at_ns:
            raise ValueError("available_at cannot precede processed_at")
        if (
            s.availability_lower_ns is not None
            and s.availability_upper_ns is not None
            and s.availability_upper_ns < s.availability_lower_ns
        ):
            raise ValueError("availability interval invalid")
        if s.availability_class == AvailabilityClass.ACTUAL_OBSERVED:
            if s.received_at_ns is None or s.available_at_ns is None:
                raise ValueError("ACTUAL_OBSERVED requires received_at and available_at")
            if s.replay_available_at_ns is not None:
            ...
            if (
                s.received_at_ns is None
                or s.replay_available_at_ns is None
                or not (s.availability_method and s.availability_method.strip())
            ):
                raise ValueError("RECONSTRUCTED_PUBLIC requires actual receipt, replay time and method")
            if s.received_at_ns <= s.replay_available_at_ns:
                raise ValueError("must not fabricate historical received_at")
        elif s.availability_class == AvailabilityClass.REVISED_NO_VINTAGE and s.replay_available_at_ns is not None:
            raise ValueError("REVISED_NO_VINTAGE must not carry replay time")
    ...
    def validate_dependencies(self, availability_by_id: Mapping[str, int]) -> None:
        ...
        latest = max(availability_by_id[d] for d in self.dependency_ids)
        ...
        if anchor is None or anchor < latest:
            raise ValueError("cannot be available before required dependencies")

    def to_dict(self) -> dict[str, Any]:
        return {
            f: (
                getattr(self, f).value
                if f == "availability_class"
                else list(getattr(self, f))
                if f == "dependency_ids"
                else getattr(self, f)
            )
...
def actual_observed(
    *,
    ...
    return InformationContract(
        ...
        processed_at_ns,
        None,
        AvailabilityClass.ACTUAL_OBSERVED,
        None,
        None,
        None,
        None,
...
def reconstructed_public(
    *,
    ...
    return InformationContract(
        ...
        None,
        None,
        AvailabilityClass.RECONSTRUCTED_PUBLIC,
        availability_lower_ns,
        availability_upper_ns,
        replay_available_at_ns,
        availability_method,
