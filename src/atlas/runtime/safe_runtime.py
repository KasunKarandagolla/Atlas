...
class SafeRuntimeConfig:
    ...
    tick_interval_ns: int = 10_000_000_000
...
class SafeRuntime:
    def __init__(self, c: SafeRuntimeConfig):
        ...
        self._last: BootResult | None = None
        self._ticks = 0
    ...
    @property
    def new_risk_allowed(self):
        return bool(self._last and self._last.new_risk_allowed)
    ...
    def _run(self) -> BootResult:
        return boot(
            journal_path=self.c.journal_path,
            lock_path=self.c.lock_path,
            capability_hash=self.c.capability_hash,
            all_qualified=self.c.all_qualified,
            assisted_enabled=self.c.assisted_enabled,
            identity_expected=self.c.identity_expected,
            identity_observed=self.c.identity_observed,
            public_health=self.c.public_health,
            private_verification=self.c.private_verification,
            now_ns=now_ns(),
            max_public_staleness_ns=self.c.max_public_staleness_ns,
            clock_uncertainty_ns=self.c.clock_uncertainty_ns,
            writer=self._writer,
            journal=self._journal,
            capability_contract=self.c.capability_contract,
            risk_policy=self.c.risk_policy,
            risk_decision=self.c.risk_decision,
            risk_snapshot_hash=self.c.risk_snapshot_hash,
            reservation_version=self.c.reservation_version,
    ...
    def _publish(self, r: BootResult):
        publish_status(
            ...
            RuntimeStatus(
                r.state.value,
                r.certificate.writer_epoch,
                True,
                r.certificate.reconciliation_health.value,
                r.unresolved_intents,
                r.unknown_commands,
                r.unresolved_commands,
                self.c.capability_hash,
                self.c.all_qualified,
                self.c.assisted_enabled,
                now_ns(),
    ...
    def tick(self) -> BootResult:
        if self._writer is None or self._journal is None:
    ...
    def run_forever(self):
        ...
        while not self._shutdown.is_set():
            self.tick()
            self._shutdown.wait(self.c.tick_interval_ns / 1e9)
        self.shutdown()
