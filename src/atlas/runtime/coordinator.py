...
class RiskPolicyDecision:
    ...
    def validates(self, policy: RiskPolicy | None, snapshot_hash: str | None, reservation_version: int | None) -> bool:
        return bool(
            ...
            and self.evidence_ref
            and self.account_snapshot_hash == snapshot_hash
            and self.reservation_version == reservation_version
            and reservation_version is not None
        )
...
def _check_capability_gate(
    ...
    if contract is None:
        return False, ["typed capability contract not supplied"]
    r = contract.assisted_blockers()
    if capability_hash != contract.contract_hash():
        r.append("capability hash does not bind contract")
    if not all_qualified:
        r.append("all_qualified false")
    if not assisted_enabled:
        r.append("assisted_enabled false")
    if not contract.assisted_enabled:
        r.append("contract assisted_enabled false")
    return not r, r
...
def boot(
    *,
    ...
    risk_decision: RiskPolicyDecision | None = None,
    risk_snapshot_hash: str | None = None,
    reservation_version: int | None = None,
    runtime_instance_id: str | None = None,
) -> BootResult:
    owned = writer or WriterLock(lock_path)
    close_writer = writer is None
    if writer is None:
        ownership = owned.acquire()
    elif writer.ownership is None:
        raise PersistenceError("writer not acquired")
    else:
        ownership = writer.ownership
    j = journal or SQLiteJournal(journal_path)
    close_journal = journal is None
    try:
        intents = j.load_unresolved_intents()
        commands = j.load_unresolved_commands()
        unknown = [
            c
            for c in commands
            if c.outcome == CommandOutcome.UNKNOWN
            or (c.send_started_at_ns is not None and c.outcome == CommandOutcome.UNSENT)
        ]
        unresolved = [
            c
            for c in commands
            if c.outcome in (CommandOutcome.UNSENT, CommandOutcome.UNKNOWN, CommandOutcome.DEFINITE_ACCEPT)
        ]
        ident = check_identity(identity_expected, identity_observed)
        public_fresh = public_health.is_fresh(
            now_ns=now_ns, max_staleness_ns=max_public_staleness_ns, clock_uncertainty_ns=clock_uncertainty_ns
        )
        private_ok = (
            private_verification.verified
            and private_verification.account_identity_hash == identity_observed.account_identity_hash
        )
        ...
        cert = RecoveryCertificate(
            uuid.uuid4().hex,
            runtime_instance_id or uuid.uuid4().hex,
            ownership.writer_id,
            ownership.writer_epoch,
            j.schema_version() or 0,
            tuple(i.intent_id for i in intents),
            tuple(c.command_id for c in unresolved),
            tuple(c.command_id for c in unknown),
        ...
        cap_ok, cap_reasons = _check_capability_gate(
            capability_contract, capability_hash, all_qualified, assisted_enabled
        )
        policy_ok = (risk_decision or RiskPolicyDecision(False)).validates(
            risk_policy, risk_snapshot_hash, reservation_version
        )
        if not policy_ok:
            cap_reasons.append("typed RiskPolicy/account snapshot/reservation decision not positively bound")
        reasons = list(ident.reasons) + cap_reasons
        if not public_fresh:
            reasons.append("public data not fresh")
        if not private_ok:
            reasons.append("private account evidence not verified")
        ...
        snap = HealthSnapshot(
            ...
            ident.ok,
            public_fresh,
            cert.reconciliation_health,
            ProtectionStatus.UNCONFIRMED,
            bool(intents),
            False,
        )
        local = new_risk_allowed(snap)
        reasons.extend(local.reasons)
        return BootResult(
            state,
            cert,
            local.allowed and cap_ok and policy_ok and private_ok,
