...
class EntryWireContract:
    ...
    def __post_init__(self):
        if (
            ...
            or self.time_in_force != "IOC"
            or self.reduce_only
            or self.position_idx != 0
            or self.sl_trigger_by != "MarkPrice"
            or self.sl_order_type != "Market"
            or self.tpsl_mode != "Full"
        ):
...
class ExitWireContract:
    ...
    def __post_init__(self):
        if self.side not in ("Buy", "Sell") or self.quantity <= 0 or not self.reduce_only or self.position_idx != 0:
        ...
        if self.order_type == "MARKET" and self.price is not None:
            raise ValueError("MARKET must not contain fake price")
    ...
    def to_bybit_params(self) -> dict[str, Any]:
        ...
        if self.price is not None:
            d["price"] = canonical_decimal_str(self.price)
        return d
...
def build_entry_wire_contract(
    ...
    return EntryWireContract(
        ...
        "Buy" if plan.side == Side.LONG else "Sell",
        quantity,
        collar_price,
        stop_price,
        client_order_id,
...
def build_exit_wire_contract(plan: TradePlan, exit_price: Decimal, quantity: Decimal) -> ExitWireContract:
    return ExitWireContract(
        f"{plan.instrument}-LINEAR.BYBIT",
        "Sell" if plan.side == Side.LONG else "Buy",
        quantity,
        "LIMIT",
        "IOC",
        exit_price,
...
def build_market_exit_wire_contract(plan: TradePlan, quantity: Decimal) -> ExitWireContract:
    return ExitWireContract(
        f"{plan.instrument}-LINEAR.BYBIT", "Sell" if plan.side == Side.LONG else "Buy", quantity, "MARKET", "IOC", None
    )
