...
def main(argv=None) -> int:
    ...
    p.add_argument("--journal", default="./atlas-journal.db")
    p.add_argument("--lock", default="./atlas-writer.lock")
    p.add_argument("--status", default="./atlas-status.json")
    p.add_argument("--run-forever", action="store_true")
    a = p.parse_args(argv)
    cap = initial_unverified_fixture()
    cfg = SafeRuntimeConfig(
        a.journal,
        a.lock,
        a.status,
        cap.contract_hash(),
    ...
    try:
        result = r.start()
        print(f"state={result.state.value} new_risk_allowed={result.new_risk_allowed}")
