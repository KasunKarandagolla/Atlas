...
class PublicVenueHealth:
    ...
    def __init__(
        self,
        state: PublicState = PublicState.DISCONNECTED,
    ...
    def is_fresh(self, *, now_ns: int, max_staleness_ns: int, clock_uncertainty_ns: int = 0) -> bool:
        ...
        if self.source_time_ns is not None and now_ns - self.source_time_ns > max_staleness_ns:
            return False
        return not (
            self.source_time_ns is not None and self.received_at_ns < self.source_time_ns - clock_uncertainty_ns
        )
...
class PrivateVerification:
    ...
    def __init__(
        ...
        *,
        ...
        verification_timestamp_ns: int | None = None,
        venue_account_identity_evidence: str | None = None,
    ) -> None:
        ...
        object.__setattr__(self, "verified_at_ns", verified_at_ns)
        object.__setattr__(self, "environment", environment)
        object.__setattr__(self, "account_identity_hash", account_identity_hash)
        self.__post_init__()
    ...
    def verified(self) -> bool:
        ...
        return (
            self.state == PrivateState.VERIFIED
            and bool(self.evidence_refs)
            and self.verified_at_ns is not None
            and self.environment == "testnet"
            and bool(identity)
            and not identity.startswith(("REQUIRED", "UNVERIFIED", "TEST_GATE"))
            if identity
            else False
        )
