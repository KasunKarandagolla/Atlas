...
class RecoveryProtectionEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.evidence_refs or any(not isinstance(x, str) or not x.strip() for x in self.evidence_refs):
...
class RecoveryCertificate:
    ...
    flat_certificate_id: str | None = None
    protection_evidence_id: str | None = None
    protection_uncertainty_summary: str = ""
    venue_observations_obtained: bool = True
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        values.setdefault("flat_certificate_id", None)
        values.setdefault("protection_evidence_id", None)
        values.setdefault("protection_uncertainty_summary", "")
        values.setdefault("venue_observations_obtained", True)
        values.setdefault("protection_evidence", None)
        values.setdefault("compatibility_metadata", {})
        for name in (
            "recovery_run_id",
            "runtime_instance_id",
            "writer_id",
    ...
    def __post_init__(self):
        ...
        if self.decision == RecoveryDecision.READY:
            ...
            if self.unresolved_intents or self.unresolved_commands or self.unknown_commands:
                raise ValueError("READY cannot contain unresolved work")
            if self.reconciliation_health != ReconciliationHealth.CURRENT:
                raise ValueError("READY requires CURRENT reconciliation")
            if not self.venue_evidence_refs:
                raise ValueError("READY requires venue evidence")
            if not (
                self.flat_certificate_id
                or self.protection_evidence_id
                or (self.protection_evidence and self.protection_evidence.evidence_refs)
            ):
                raise ValueError("READY requires persisted flat/protection artifact")


def _validate_resolution_artifact(
    *,
    journal: Any,
    flat_certificate_id: str | None,
    ...
    now_ns: int,
    max_protection_staleness_ns: int,
) -> tuple[bool, list[str]]:
    reasons = []
    if flat_certificate_id:
        valid, artifact_reasons = journal.validate_flat_certificate_for_recovery(
            flat_certificate_id,
        ...
        valid, artifact_reasons = journal.validate_protection_evidence_for_recovery(
            protection_evidence_id,
            reconciliation_run_id=reconciliation_run_id,
            account=account,
            instrument=instrument,
        ...
        reasons.append("no flat/protection artifact")
    return not reasons, reasons


def _residual_risk_reasons(bundle: Any) -> list[str]:
    """Return positive residual-risk facts that prevent READY."""
    from .reconciliation_evidence import QueryType

    by_type = {query.query_type: query for query in bundle.queries}
    reasons = []
    open_orders = by_type.get(QueryType.OPEN_ORDERS)
    if open_orders is not None and open_orders.total_records_returned > 0:
        reasons.append("unexpected remaining opening order")
    conditional = by_type.get(QueryType.CONDITIONAL_ORDERS)
    if (
        conditional is not None
        and conditional.total_records_returned > 0
        and str(conditional.facts.get("is_current_protection", "false")).lower() not in {"true", "1", "yes"}
    ):
        reasons.append("unexpected conditional order")
    for query in bundle.queries:
        for key in ("unexpected_remaining_open_orders", "unexpected_conditional_orders", "unresolved_opening_command"):
            if str(query.facts.get(key, "false")).lower() in {"true", "1", "yes"}:
                reasons.append(key.replace("_", " "))
    return reasons


def recover_from_persisted_run(
    *,
    journal: Any,
    recovery_run_id: str,
    reconciliation_run_id: str,
    ...
    if (
        run.account != account
        or run.instrument != instrument
        or run.writer_id != writer_id
        or run.writer_epoch != writer_epoch
    ...
    artifact_ok, artifact_reasons = _validate_resolution_artifact(
        ...
        protection_evidence_id=protection_evidence_id,
        reconciliation_run_id=reconciliation_run_id,
        account=account,
        instrument=instrument,
        position_epoch=position_epoch,
        ...
        now_ns=ended_at_ns,
        max_protection_staleness_ns=max_protection_staleness_ns,
    )
    actual_intents = tuple(i.intent_id for i in journal.load_unresolved_intents())
    actual_commands = tuple(
        c.command_id
        for c in journal.load_unresolved_commands()
        if c.outcome.value in {"UNSENT", "UNKNOWN", "DEFINITE_ACCEPT"}
    )
    ...
    for supplied, actual, label in (
        (unresolved_intent_ids, actual_intents, "intents"),
        (unresolved_command_ids, actual_commands, "commands"),
        (unknown_command_ids, actual_unknown, "unknown commands"),
    ):
        if supplied is not None and tuple(supplied) != actual:
    ...
    cert = RecoveryCertificate(
        ...
        writer_id,
        writer_epoch,
        journal.schema_version() or 0,
        tuple(unresolved_intent_ids),
        tuple(unresolved_command_ids),
        tuple(unknown_command_ids),
        health,
        started_at_ns,
        ended_at_ns,
...
def run_recovery(
    *,
    ...
    prerequisites_ok: bool,
    protection_evidence: RecoveryProtectionEvidence | None = None,
    reconciliation_evidence: Any = None,
) -> RecoveryCertificate:
    ...
    """
    refs = (
        tuple(getattr(q, "evidence_hash", q) for q in getattr(reconciliation_evidence, "queries", ()))
    ...
    if reconciliation_health == ReconciliationHealth.CONFLICTED:
        ...
        decision = (
            ...
            if reconciliation_health != ReconciliationHealth.CURRENT and (unresolved_command_ids or unknown_command_ids)
            else RecoveryDecision.REMAIN_RECOVERING
        )
    else:
    ...
    return RecoveryCertificate(
        ...
        evidence_refs=("journal-restore",) + refs,
        venue_evidence_refs=tuple(venue_evidence_refs) if venue_observations_obtained else (),
        decision=decision,
...
def recover_from_persisted_evidence(
    *,
    ...
    cert = run_recovery(
        ...
        writer_id=writer_id,
        writer_epoch=writer_epoch,
        journal_schema_version=journal_schema_version or journal.schema_version() or 0,
        ...
        started_at_ns=started_at_ns,
        ended_at_ns=ended_at_ns,
        venue_observations_obtained=bool(queries),
        venue_evidence_refs=tuple(e.evidence_hash for e in queries),
