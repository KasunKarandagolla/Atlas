...
def obs(at):
    return ProtectionObservation(
        0, 1, Decimal("0.01"), "MarkPrice", Decimal("48000"), "Full Market ReduceOnly", ("position-view",), at
    )
...
def test_future_source_time_fails_closed_without_fabricating_receipt():
    result = verify_protection(
        ...
        instrument="BTCUSDT",
        receive_time_ns=T0,
        conditional_order_evidence_ids=("conditional-view",),
...
def test_typed_protection_roundtrip_is_restart_safe(journal):
    ...
    journal.append_protection_evidence("pe-1", result.evidence, result.evidence.expected_evidence_hash())
    restored = journal.load_protection_evidence("pe-1")
