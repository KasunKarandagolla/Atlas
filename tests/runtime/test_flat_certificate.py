...
def test_flat_certificate_allows_historical_fills_when_current_position_is_flat():
    ...
    certificate = certify_flat(
        ...
        writer_id="writer-1",
        writer_epoch=1,
        account_identity_hash="acct-hash",
        instrument="BTCUSDT",
        position_epoch=0,
        zero_position_evidence=_query(QueryType.POSITIONS, "position"),
        opening_commands_evidence=[_query(QueryType.ORDER_HISTORY, "opening")],
        ...
        economic_evidence=_query(QueryType.TRANSACTION_LOG, "economic", records=1),
        now_ns=T0 + 3,
        current_signed_position_qty=Decimal("0"),
        current_position_evidence_complete=True,
        terminal_opening_order_certainty=True,
        no_unresolved_opening_command=True,
        reservation_release_evidence=True,
        writer_identity_current=True,
        account_identity_current=True,
        required_query_windows_complete=True,
    )
    assert certificate.decision == FlatCertificationDecision.CERTIFIED_FLAT
    assert certificate.decision == FlatCertificationDecision.CERTIFIED_FLAT
    assert not certificate.can_release_reservation
...
def test_late_contradictory_evidence_reopens_recovery():
    certificate = certify_flat(
        ...
        economic_evidence=_query(QueryType.TRANSACTION_LOG, "economic"),
        now_ns=T0 + 3,
        current_position_evidence_complete=True,
        terminal_opening_order_certainty=True,
        no_unresolved_opening_command=True,
...
def test_reservation_release_consumes_valid_flat_certificate():
    ...
    try:
        certificate = certify_flat(
            certification_id="cert-release",
            reconciliation_run_id="recon-release",
            writer_id="writer-1",
            writer_epoch=1,
            account_identity_hash="acct-hash",
            instrument="BTCUSDT",
            ...
            no_residual_protection_orders_evidence=simulator._query(
                QueryType.CONDITIONAL_ORDERS, query_id="conditional-release"
            ),
            execution_evidence=build_execution_evidence(
                "intent-fault", simulator.journal.load_intent("intent-fault").client_order_id, [], [], T0 + 2
            ),
            economic_evidence=simulator._query(QueryType.TRANSACTION_LOG, query_id="economic-release"),
            now_ns=T0 + 3,
            current_position_evidence_complete=True,
            terminal_opening_order_certainty=True,
            no_unresolved_opening_command=True,
        ...
        with pytest.raises(PersistenceError, match="flat certificate"):
            simulator.journal.release_reservation_from_flat_certificate(certificate)
        assert simulator.journal.load_reservation("intent-fault").remaining_open_qty == Decimal("0.010")
    finally:
        simulator._close()
