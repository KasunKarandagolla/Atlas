...
def cfg(tmp_path):
    ...
    return SafeRuntimeConfig(str(tmp_path/'j.db'),str(tmp_path/'w.lock'),str(tmp_path/'status.json'),c.contract_hash(),False,False,IdentityExpectation(),ObservedAccountState('testnet','BYBIT','linear','one_way','isolated','REQUIRED',('BTCUSDT','ETHUSDT'),False),PublicVenueHealth(),PrivateVerification(),5_000_000_000,capability_contract=c)

def test_runtime_identity_stable_recovery_cycle_identity_distinct(tmp_path):
    r=SafeRuntime(cfg(tmp_path));first=r.start();journal=r.journal;rid=r.runtime_instance_id;second=r.tick()
    ...
    r.shutdown();assert r.journal is None and not r.writer_held

def test_capability_evidence_requires_real_refs_and_profile_hash():
    ledger=CapabilityEvidenceLedger();name=ledger.REQUIRED_CAPABILITIES[0]
    with pytest.raises(ValueError):ledger.record_offline_test(name,'run',(),1,True)
    with pytest.raises(ValueError):ledger.record_testnet_gate(name,'run',('evidence',),1,True,target_profile_hash='bad')
