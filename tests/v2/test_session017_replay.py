...
def replay_context(
    ...
    *,
    ...
    decision_cutoff_ns=None,
):
    cutoff = CUTOFF if decision_cutoff_ns is None else decision_cutoff_ns
    if action_override is None:
        ...
        if (
            action.candidate_ref != case.candidate.content_hash
            or action.candidate_set_ref != case.candidate_set.content_hash
        ):
            raise ValueError("replay action does not match the selected candidate fixture")
    manifest = source(repo, "scenario-manifest", cutoff)
    paths = common_paths(
        seed=path_seed,
        experiment_ref=source(repo, "experiment", CUTOFF),
        decision_ref=case.candidate_set.content_hash,
        count=1,
    )
    ...
    schedule = FundingScheduleV2(
        cutoff, tuple(expected_funding), not expected_funding, source(repo, "funding-schedule", cutoff)
    )
    ...
    path = ReplayPathV2(
        ...
        tuple(closed15m),
        tuple(funding),
        source(repo, "replay-path", cutoff),
    )
...
def run(repo, case, context):
    ...
    return replay_action(
        ...
        candidate=case.candidate,
        path=path,
        existing_portfolio_ref=existing.content_hash,
        assumptions=assumptions,
        fee=case.fee,
        schedule=schedule,
        product=case.product,
        replay_cutoff_ns=path.available_at_ns,
    )
...
def test_partial_no_fill_outside_collar_missing_depth_and_arrival(tmp_path):
    ...
    with OpsRepository(tmp_path / "arrival.sqlite") as repo:
        ...
        context = replay_context(
            ...
            assumptions=delayed,
            minutes=(
                minute(CUTOFF, ask="100"),
                minute(CUTOFF + MINUTE_NS, ask="101"),
    ...
    with OpsRepository(tmp_path / "late.sqlite") as repo:
        ...
        context = replay_context(
            ...
            case,
            assumptions=late,
            minutes=(minute(CUTOFF), minute(CUTOFF + MINUTE_NS), minute(CUTOFF + 4 * HOUR_NS)),
        )
        result = run(repo, case, context)
...
def test_mark_stop_gap_after_latency_and_unresolved_residual(tmp_path):
    with OpsRepository(tmp_path / "stop.sqlite") as repo:
        ...
        minutes = (
            minute(CUTOFF),
            minute(
                CUTOFF + MINUTE_NS, bid="97", ask="98", mark_low="98", mark_high="101", last_low="96", last_high="101"
            ),
            minute(
                CUTOFF + 2 * MINUTE_NS, bid="97", ask="98", mark_low="97", mark_high="99", last_low="95", last_high="99"
            ),
        )
    ...
    with OpsRepository(tmp_path / "residual.sqlite") as repo:
        case = risk_case(repo)
        context = replay_context(repo, case, minutes=(minute(CUTOFF), minute(CUTOFF + 4 * HOUR_NS, bid_depth="0")))
        result = run(repo, case, context)
        assert result.status == ReplayStatusV2.NOT_ESTIMABLE
...
def test_signed_funding_on_partial_fill_and_missing_support(tmp_path):
    with OpsRepository(tmp_path / "funding.sqlite") as repo:
        ...
        context = replay_context(
            ...
            funding=(settlement,),
            expected_funding=(at,),
            minutes=(minute(CUTOFF, ask_depth="0.5"), minute(CUTOFF + 4 * HOUR_NS, bid="101", ask="101")),
        )
        result = run(repo, case, context)
    ...
    with OpsRepository(tmp_path / "missing.sqlite") as repo:
        case = risk_case(repo)
        context = replay_context(
            repo, case, expected_funding=(CUTOFF + HOUR_NS,), minutes=(minute(CUTOFF), minute(CUTOFF + 4 * HOUR_NS))
        )
...
def test_partial_stop_exits_charge_each_fill_once_and_fund_surviving_quantity(tmp_path):
    with OpsRepository(tmp_path / "ops.sqlite") as repo:
        ...
        settlement = FundingCashflowV2(
            settlement_at,
            settlement_at,
            Decimal("0.01"),
            Decimal("100"),
            source(repo, "partial-exit-settlement", settlement_at),
        )
        context = replay_context(
            repo,
            case,
            funding=(settlement,),
            expected_funding=(settlement_at,),
            minutes=(
                minute(CUTOFF, ask_depth="1"),
                minute(CUTOFF + MINUTE_NS, mark_low="98", mark_high="101", last_low="96"),
                minute(CUTOFF + 2 * MINUTE_NS, bid="97", ask="98", bid_depth="0.4", mark_low="97", last_low="95"),
                minute(CUTOFF + 4 * MINUTE_NS, bid="97", ask="98", bid_depth="0.6", mark_low="97", last_low="95"),
            ),
        )
        result = run(repo, case, context)
        assert result.status == ReplayStatusV2.PARTIAL_FILL
...
def test_future_funding_artifact_cannot_rewrite_prior_action_or_payoff(tmp_path):
    with OpsRepository(tmp_path / "ops.sqlite") as repo:
        case = risk_case(repo)
        context = replay_context(
            repo, case, minutes=(minute(CUTOFF), minute(CUTOFF + 4 * HOUR_NS, bid="101", ask="101"))
        )
        action, _, _, _, path = context
        original = run(repo, case, context)
        future_at = path.available_at_ns + HOUR_NS
        future = FundingCashflowV2(
            future_at, future_at, Decimal("0.9"), Decimal("999"), source(repo, "future-funding", future_at)
        )
...
def s2_replay_context(repo, *, first_close="101", second_close="101"):
    ...
    universe = UniverseContractV2(
        ArtifactEnvelope(1, "s2-replay-u", t, t, "fixture", (product.content_hash,)),
        "fixture",
        t,
    ...
    candidate_set = assemble_candidate_set(
        repo,
        universe=universe,
        decision_event_id="S2_EVENT",
        cutoff_ns=t,
        candidates=(item,),
        policies={S2_POLICY.policy_hash: S2_POLICY},
    ...
    v2 = RiskPolicyV2("SESSION017_S2_FIXTURE", 0, v1.policy_hash(), Decimal("0.05"), Decimal("0.02"), 1)
    ...
    venue = VenueSizingLimitsV2(
        KEY, product.content_hash, t, (Decimal("1"),), Decimal("1"), Decimal("0"), source(repo, "s2-venue", t)
    )
    ...
    path_id = common_paths(
        seed=18, experiment_ref=source(repo, "s2-experiment", t), decision_ref=item.content_hash, count=1
    ).path_ids[0]
...
def test_s2_failed_break_first_second_and_third_bar_exclusion(tmp_path):
    with OpsRepository(tmp_path / "ops.sqlite") as repo:
        item, product, action, existing, assumptions, fee, schedule, manifest, path_id, first, second = (
            s2_replay_context(repo, first_close="100", second_close="101")
        )
        t = item.decision_at_ns
        mins = (
            minute(t, ask="100.6", bid="100.59"),
            minute(t + 15 * MINUTE_NS, bid="100", ask="100.01"),
            minute(t + 16 * MINUTE_NS, bid="99.9", ask="100"),
            minute(t + 30 * MINUTE_NS, bid="101", ask="101.01"),
        )
        path = ReplayPathV2(
            path_id, manifest, t + 31 * MINUTE_NS, mins, (first, second), (), source(repo, "s2-path", t)
        )
        index_replay_path(repo, path)
        ...
        for seed, first_close, second_close, should_fail in scenarios:
            another_id = common_paths(
                seed=seed,
                experiment_ref=source(repo, f"s2-experiment-{seed}", t),
                decision_ref=item.content_hash,
                count=1,
            ).path_ids[0]
            another = ExistingPortfolioPathV2(another_id, manifest, t, t, source(repo, f"s2-portfolio-{seed}", t), ())
            ...
            later_path = ReplayPathV2(
                another_id,
                manifest,
                t + 2 * HOUR_NS + MINUTE_NS,
                later_minutes,
                bars,
                (),
                source(repo, f"s2-path-{seed}", t),
            )
            ...
            later = replay_action(
                ...
                candidate=item,
                path=later_path,
                existing_portfolio_ref=another.content_hash,
                assumptions=assumptions,
                fee=fee,
                schedule=schedule,
                product=product,
